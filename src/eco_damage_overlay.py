...
from eco_paths import resolve_dirs, ensure_data_layout, find_data_file, log_dir, resource_path

_RES_DIR, _DATA_DIR = resolve_dirs(__file__)
ensure_data_layout(_DATA_DIR)
HERE = _DATA_DIR
LOGDIR = log_dir(_DATA_DIR)
CONFIG_PATH = find_data_file(_DATA_DIR, _RES_DIR, "damage_overlay_config.json")
TRANSPARENT = "#010203"
...
def main():
    ...
    if not ecos:
        print("eco.exe is not running", flush=True)
        return 1
    ...
    js = open(resource_path(_RES_DIR, "_damage_capture.js"), encoding="utf-8").read()
