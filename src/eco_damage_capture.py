...
# Keep interactive status prints on stdout; structured logger uses stderr.
logger = setup_logger(
    "eco_damage_capture",
    log_dir=LOGDIR,
    log_file="eco_damage_capture.log",
    stream=sys.stderr,
)
...
def load_op_names():
    paths = [
        os.path.join(_DATA_DIR, "opcodes.json"),
        os.path.join(_RES_DIR, "opcodes.json"),
        os.path.join(_RES_DIR, "..", "archive", "opcodes.json"),
        os.path.join(_DATA_DIR, "..", "archive", "opcodes.json"),
    ]
    for path in paths:
...
class DamageCapture:
    ...
    def on_message(self, message, data):
        ...
        if payload == "READY":
            logger.info("[*] damage capture ready. F8=mark, F9=stats, Ctrl+C=stop")
        ...
        typ = payload.get("t")
        if typ == "key":
            logger.info("[*] AES key candidate #%s seen", payload.get("n"))
            return
        if typ == "lock":
            logger.info("[*] stream locked dir=%s port=%s", payload.get("dir"), payload.get("port"))
            return
        if typ != "pkt":
            return
...
def main():
    ...
    js_path = resource_path(_RES_DIR, "_damage_capture.js")
