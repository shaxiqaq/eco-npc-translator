...
// Precomputed hex nibble table for hot-path packet hex encoding.
const HEX = [];
for (let i = 0; i < 256; i++) HEX[i] = (i < 16 ? '0' : '') + i.toString(16);
function hex(a) {
  let s = '';
  for (let i = 0; i < a.length; i++) s += HEX[a[i] & 0xff];
  return s;
}
function fromHex(h) {
  const n = h.length >> 1;
  const a = new Array(n);
  for (let i = 0, j = 0; i < n; i++, j += 2) a[i] = parseInt(h.substr(j, 2), 16);
  return a;
}
...
// keyStr -> expanded round key (avoid re-expand per frame)
const KEYMAP = {};
let KEY_COUNT = 0;
let RK = null;
const INJECT_QUEUE = [];
