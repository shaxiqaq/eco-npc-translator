...
def main():
    ...
    print("客户端名称表导入完成")
    print("====================")
    print(f"怪物名称: {len(mob_names)} 条，新导入 {mob_added} 条 -> {MOB_OUT}")
    print(f"技能名称: {len(skill_names)} 条，新导入 {skill_added} 条 -> {SKILL_OUT}")
    if seen_files:
        print()
        print("使用到的 CSV:")
        for kind, path, count in seen_files:
            print(f"  {kind}: {path} ({count} 条候选)")
    else:
        print()
        print("没有找到可导入的怪物/技能 CSV。")
        print(f"客户端原始表在: {DEFAULT_XLS_DIR}")
        print("那里目前只有 table.hed/table.dat，需要先用 uneco 或同类工具导出 CSV。")
