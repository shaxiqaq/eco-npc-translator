...
HERE = _DATA_DIR
LOGDIR = log_dir(_DATA_DIR)
# Log to stderr so JSON-lines bridges that use stdout stay machine-readable.
logger = setup_logger(
    "eco_damage_meter",
    log_dir=LOGDIR,
    log_file="eco_damage_meter.log",
    stream=sys.stderr,
)
WATCH_OPS = [
...
class DamageMeter:
    ...
    def reload_custom_durations(self, durations=None):
        """Reload custom buff durations from disk, or apply an explicit map."""
        with self.lock:
            if isinstance(durations, dict):
                self.buff_tracker.set_custom_durations(durations)
            else:
                self.buff_tracker.reload_custom_durations()
            return dict(self.buff_tracker.custom_durations)
    ...
    def find_recent_own_buff_skill(self, ts, actor, max_age=1.75):
        for action in list(self.recent_actions):
            age = ts - action["ts"]
            if age < 0 or age > max_age:
                continue
            if action.get("kind") != "skill" or not action.get("own"):
                continue
            if action.get("buff_match_used"):
                continue
            target = action.get("target")
            if target not in (None, 0, actor):
                continue
