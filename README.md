...
- **伤害统计**：抓取战斗封包，显示自己造成的伤害、受到的伤害、技能伤害、普通攻击伤害，以及宠物造成的伤害。
- **角色状态监控**：显示自己角色当前的增益、减益和异常状态，并通过透明悬浮窗显示持续时间。
...
```text
electron/release/ECO-Toolbox-Setup-0.2.4.exe
```

安装后的主界面使用左侧菜单组织全部功能：

- `总览`：启动或停止伤害采集、NPC 翻译和状态悬浮窗。
...
状态悬浮窗是 Electron 透明窗口，平时保持置顶和鼠标穿透；在主界面点击`调整状态悬浮窗`后可以拖动，点击`完成调整`会保存位置。已实测持续时间的状态显示预计倒计时，未知持续时间的状态显示已经持续的时间。
...
- 状态监控依赖当前已解析的状态位；尚未验证的状态名称或持续时间可能需要继续采样修正。
